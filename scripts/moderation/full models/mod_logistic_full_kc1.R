...
for (k in 1:length(listcsv)){
  ...
  name = paste("/cloud/project/output/moderation/KC1/",filename)
  print(name) 
  sink(file = name)
  ...
  df <- ldf[[k]] %>% select(c(7,8,10,11,12,14)) %>% mutate(LOC=log(loc+1))
  
  cat("***Moderation Continuous Full Model***\n")
  cat("----------------------------------------\n")
  
  model <- glm( data= df , buggy ~ (rfc + wmc + cbo + lcom)*LOC ,family = "poisson")
