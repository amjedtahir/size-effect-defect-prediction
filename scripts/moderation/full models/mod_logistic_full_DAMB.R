...
for (k in 1:length(listcsv)){
  ...
  name = paste("/cloud/project/output/moderation/DAMB/full/logisitc/",filename)
  print(name) 
  sink(file = name)
  ...
  df <- ldf[[k]] %>% select(c(2,3,4,5,7,8,9,10,24)) %>% mutate(LOC=log(loc+1))
  
  cat("***Moderation Continuous Full Model***\n")
  cat("----------------------------------------\n")
  
  model <- glm( data= df , buggy ~ (rfc + wmc + cbo + lcom + fanin + fanout)*LOC ,family = "binomial")
